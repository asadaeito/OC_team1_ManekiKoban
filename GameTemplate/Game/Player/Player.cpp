...
Player::Player():m_currentHp(LIFE_MAX)
{
	...
	/*NOTE:OCの問題点として「移動アニメーションがない」があるからここを消しておく*/
...
bool Player:: Start()
{
	m_itemSpeed = FindGO<Item_Speed>("item_speed");
...
void Player::Move()
{
	...
	if (m_characterController.IsOnGround())
	{
		...
		if (g_pad[0]->IsPress(enButtonB))
		{
			...
			else
			{
				...
				//g_soundEngine->ResistWaveFileBank(3, "Assets/SE/Dash.wav");
		...
		if (g_pad[0]->IsTrigger(enButtonA))
		{
			//ジャンプ処理
			//効果音処理
			g_soundEngine->ResistWaveFileBank(2, "Assets/SE/Jump.wav");
	...
	/*NOTE:OCの問題点で２段ジャンプが必要なので後で消しておく*/
	//二段ジャンプ処理
	if (g_pad[0]->IsTrigger(enButtonA))
	{
		//効果音処理
